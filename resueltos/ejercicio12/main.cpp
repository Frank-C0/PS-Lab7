// Ejercicio 12
// Crear un programa en C++ que capture la excepción
// de crear un logaritmo de un número negativo o el
// logaritmo de 0
...
const char *ErrorMat::porque() const throw()
{
    ...
    case ERANGO:
        ...
        break;
    default:
        return "Error Desconocido"; // En rigor no debería ocurrir tal Excepción
    }
}
double logaritmo(const double n)
{
    ...
    {
        if (n < 0)
            throw(ErrorMat(EDOMINIO));
        if (n == 0)
            throw(ErrorMat(ERANGO));
        return log(n);
    }
    catch (ErrorMat &e)
    {
